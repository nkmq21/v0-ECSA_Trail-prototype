...
create
or replace function public.seed_purchased_plan_stops()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
...
create
or replace function public.handle_plan_purchase()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
update public.plan
...
create
or replace function public.handle_new_review()
returns trigger language plpgsql security definer set search_path = public
as $$
declare
v_creator_id uuid;
...
update public.plan
set review_count = v_old_count + 1,
...
set review_count = review_count + 1,
    rating       = (select coalesce(avg(p.rating), 0)
                    from public.plan p
...
create
or replace function public.handle_plan_status_change()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
    if
new.status = 'published' and old.status <> 'published' then
update public.profile
...
values (new.creator_id, new.id, null) on conflict (user_id, plan_id) do nothing;

elsif
old.status = 'published' and new.status <> 'published' then
update public.profile
set total_plans = greatest(total_plans - 1, 0)
where id = new.creator_id;
end if;
return new;
end;
...
create
policy "profile_select_all"  on public.profile for
select using (true);
create
policy "profile_update_own"  on public.profile for
update to authenticated
    using ((select auth.uid()) = id);

-- place + place_source: public read; writes via service role only (ingestion pipeline)
create
policy "place_select_all"        on public.place        for
select using (true);
create
policy "place_source_select_all" on public.place_source for
select using (true);

-- plan: published plans + own drafts visible; mutations only by creator
create
policy "plan_select" on public.plan for
select
    using (status = 'published' or (select auth.uid()) = creator_id);
create
policy "plan_insert" on public.plan for insert to authenticated
    with check ((select auth.uid()) = creator_id);
create
policy "plan_update" on public.plan for
update to authenticated
    using ((select auth.uid()) = creator_id);
create
policy "plan_delete" on public.plan for delete
to authenticated
    using ((select auth.uid()) = creator_id);
...
create
policy "stop_plan_select" on public.stop for
select
    using (
...
create
policy "stop_plan_write" on public.stop for all to authenticated
    using (
...
create
policy "stop_purchased_all" on public.stop for all to authenticated
    using (
...
create
policy "stop_alert_select" on public.stop for
...
create
policy "purchased_plan_select" on public.purchased_plan for
select to authenticated
    using ((select auth.uid()) = user_id);
create
policy "purchased_plan_insert" on public.purchased_plan for insert to authenticated
    with check ((select auth.uid()) = user_id);

-- trip: own rows only
create
policy "trip_all" on public.trip for all to authenticated
    using ((select auth.uid()) = user_id);

-- weather_alert: readable/updatable by trip owner
create
policy "weather_alert_select" on public.weather_alert for
select to authenticated
    using (exists (
    select 1 from public.trip t where t.id = trip_id and (select auth.uid()) = t.user_id
    ));
create
policy "weather_alert_update" on public.weather_alert for
update to authenticated
    using (exists (
    select 1 from public.trip t where t.id = trip_id and (select auth.uid()) = t.user_id
    ));
...
create
policy "review_select_all" on public.review for
select using (true);
create
policy "review_insert"     on public.review for insert to authenticated
    with check ((select auth.uid()) = user_id);
create
policy "review_update"     on public.review for
update to authenticated
    using ((select auth.uid()) = user_id);
create
policy "review_delete"     on public.review for delete
to authenticated
    using ((select auth.uid()) = user_id);

-- chat_session + chat_message: own rows only
create
policy "chat_session_all" on public.chat_session for all to authenticated
    using ((select auth.uid()) = user_id);
create
policy "chat_message_all" on public.chat_message for all to authenticated
    using (exists (
...
create
policy "ai_subscription_select" on public.ai_subscription for
select to authenticated
    using ((select auth.uid()) = user_id);
...
select p.province,
       ...
       count(distinct ps.id)              filter (where ps.source_type = 'facebook')               as facebook_hits, count(distinct ps.id) filter (where ps.source_type = 'google_places')          as google_maps_hits, count(distinct ps.id) filter (where ps.source_type = 'blog')                   as blog_hits
from public.place p
