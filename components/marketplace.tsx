...
import { AiServiceModal } from '@/components/ai-service-modal'
...
function BusinessBanner() {
  ...
  return (
    <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-accent/10 border border-primary/20 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      ...
      <Button variant="outline" size="sm" className="rounded-lg text-xs flex-shrink-0 border-primary/30 text-primary hover:bg-primary/10">
...
export function Marketplace({ onOpenInPlanner }: MarketplaceProps) {
  ...
  const [ownedIds, setOwnedIds] = useState<Set<string>>(new Set(['plan-4'])) // demo: plan-4 pre-owned
  ...
  function handleAiComplete(tier: AiServiceTier | null) {
    if (buyingPlan) {
      setOwnedIds(prev => new Set([...prev, buyingPlan.id]))
    }
          ...
          <AnimatePresence mode="popLayout">
            {filtered.length === 0 ? (
              ...
              >
                ...
                <Button variant="ghost" size="sm" onClick={() => { setSearch(''); setCategory('all') }}>Clear filters</Button>
              </motion.div>
            ) : (
              <motion.div
                key="grid"
